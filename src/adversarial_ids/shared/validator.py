...
def validate_and_clamp_attack_config(
    ...
    # --------------------------------------------------------------------- #
    # Passes genéricos — valem para QUALQUER tipo de ataque do ERENO.       #
    # (Rodam por último, para garantir coerência sobre os valores finais.)  #
    # --------------------------------------------------------------------- #
    warnings.extend(_clamp_probabilities(config))
    warnings.extend(_enforce_min_le_max(config))

    return config, warnings


# Substrings que identificam uma folha como probabilidade (case-insensitive):
# ``prob``, ``spikeProb``, ``reorderProb``, ``burstDropProb``, ``sqnumResetProb``,
# ``ethSrcSpoofProb``, ``srcProb``/``dstProb``, ``selectionProb``, etc.
_PROB_KEY_MARKER = "prob"


def _clamp_probabilities(config: dict[str, Any]) -> list[str]:
    """Garante que toda folha ``*prob*`` numérica fique em [0, 1]."""
    warnings: list[str] = []
        ...
        for key, value in node.items():
            ...
            if isinstance(value, dict):
                ...
                if clamped != value:
                    node[key] = clamped
                    warnings.append(
                        f"{child_path} ajustado de {value} para {clamped}, "
...
def _enforce_min_le_max(config: dict[str, Any]) -> list[str]:
    """Em todo objeto com ``min`` e ``max`` numéricos, garante ``min <= max``."""
    warnings: list[str] = []
    ...
    def walk(node: Any, path: str) -> None:
        ...
        if (
            ...
            node["min"] = high
            warnings.append(
                f"{path + '.' if path else ''}min ajustado de {low} para {high}, "
                "pois estava maior que max."
            )
        for key, value in node.items():
            child_path = f"{path}.{key}" if path else key
            walk(value, child_path)
