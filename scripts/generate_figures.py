...
def read_model_metrics(folder: str) -> pd.DataFrame:
    ...
    if not path.exists():
        ...
        return pd.DataFrame(columns=["iteration", "f1_score_attack"])
    ...
    df["f1_score_attack"] = pd.to_numeric(
        df["f1_score_attack"],
        errors="coerce",
    )

    df = df.dropna(subset=["iteration", "f1_score_attack"])
    df["iteration"] = df["iteration"].astype(int)

    return df[["iteration", "f1_score_attack"]].sort_values("iteration")
...
def build_wide_csv() -> pd.DataFrame:
    ...
    for folder, meta in MODELS.items():
        ...
        df = df.rename(columns={"f1_score_attack": meta["label"]})
