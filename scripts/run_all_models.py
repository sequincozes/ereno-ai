...
# scripts/ vive na raiz do repositório: subir um nível.
...
def run_compare_configs() -> None:
    compare_script = BASE_DIR / "scripts" / "compare_attack_configs.py"

    if not compare_script.exists():
        log("[WARN] scripts/compare_attack_configs.py não existe. Pulando comparação.")
        return

    result = subprocess.run(
        ["python", "scripts/compare_attack_configs.py"],
        cwd=BASE_DIR,
        text=True,
    )
...
def run_model(model_id: str, iterations: int) -> None:
    ...
    command = [
        "python",
        "-m",
        "adversarial_ids.interfaces.cli",
        "--model-id",
        model_id,
        "--iterations",
        str(iterations),
